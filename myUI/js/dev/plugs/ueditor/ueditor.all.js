...
UE.commands['preview'] = {
    execCommand : function(){
        var w = window.open('', '_blank', ''),
            d = w.document;
        d.open();
        d.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en"><head><script src="'+this.options.UEDITOR_HOME_URL+'ueditor.parse.js"></script><script src="'+this.options.UEDITOR_HOME_URL+'jwplayer.js"></script><script>' +
            ...
            "})},300)" +
            '</script></head><body><div>'+UE.utils.html(this.getContent(null,null,true))+'</div></body></html>');
        d.close();
    },
    notNeedUndo : 1
};
...
UE.plugins['video'] = function (){
    ...
    function creatInsertStr(url,width,height,id,align,type,toEmbed){
        return  !toEmbed ?
                '<img ' + (id ? 'id="' + id+'"' : '') + ' width="'+ width +'" height="' + height + '" _url="'+url+'" class="edui-faked-video"'  +
                ' src="' + me.options.UEDITOR_HOME_URL+'themes/default/images/spacer.gif" filetype="'+type+'" style="background:url('+me.options.UEDITOR_HOME_URL+'themes/default/images/videologo.gif) no-repeat center center; border:1px solid gray;'+(align ? 'float:' + align + ';': '')+'" />'
                :
                '<embed type="application/x-shockwave-flash" class="edui-faked-video" id="'+ id +'" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
                    ' src="' + url + '" filetype="'+ type +'" width="' + width  + '" height="' + height  + '"'  + (align ? ' style="float:' + align + '"': '') +
                    ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" >'
                ;
    }
    ...
    function switchImgAndEmbed(root,img2embed){
        utils.each(root.getNodesByTagName(img2embed ? 'img' : 'embed'),function(node){
            if(/playerkelRagfMuWSi/.test(node.getAttr('id'))){
                var html = creatInsertStr( img2embed ? node.getAttr('_url') : node.getAttr('src'),node.getAttr('width'),node.getAttr('height'),node.getAttr('id'),node.getStyle('float') || '',node.getAttr('filetype'),img2embed);
    ...
    me.commands["insertvideo"] = {
        execCommand: function (cmd, videoObjs){
            ...
            for(var i=0,vi,len = videoObjs.length;i<len;i++){
                vi = videoObjs[i];
                objId = 'playerkelRagfMuWSi'+ (new Date()-0);
                html.push(creatInsertStr( vi.url, vi.width || 480,  vi.height || 270, objId,null,vi.type,false));
            }
...
UE.plugins['music'] = function () {
     ...
     */
    function creatInsertStr(url,width,height,id,align,toEmbed,addParagraph){
        return  !toEmbed ?
            (addParagraph? ('<p '+ (align !="none" ? ( align == "center"? ' style="text-align:center;" ':' style="float:"'+ align ) : '') + '>'): '') +
                '<img align="'+align+'" width="'+ width +'" height="' + height + '" _url="'+url+'"id="'+id+'" class="edui-faked-music"' +
                ' src="'+me.options.langPath+me.options.lang+'/images/music.png" />' +
                (addParagraph?'</p>':'')
            :
            '<embed type="application/x-shockwave-flash" id="'+id+'" class="edui-faked-music" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
    ...
    /*function creatInsertStr(url,width,height,id,align,toEmbed,addParagraph){
        return  !toEmbed ?
            (addParagraph? ('<p '+ (align !="none" ? ( align == "center"? ' style="text-align:center;" ':' style="float:"'+ align ) : '') + '>'): '') +
                '<img id="'+id+'" align="'+align+'" width="'+ width +'" height="' + height + '" _url="'+url+'" class="edui-faked-music"' +
                ' src="'+me.options.langPath+me.options.lang+'/images/music.png" />' +
                (addParagraph?'</p>':'')
            :
            '<embed type="application/x-shockwave-flash" class="edui-faked-music" id="'+id+'" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
                ' src="' + url + '" width="' + width  + '" height="' + height  + '" align="' + align + '"' +
                ( align !="none" ? ' style= "'+ ( align == "center"? "display:block;":" float: "+ align )  + '"' :'' ) +
                ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" >';
    }*/
    ...
    function switchImgAndEmbed(img2embed) {
        ...
        for (var i = 0, node; node = nodes[i++];) {
            ...
            tmpdiv.innerHTML = creatInsertStr(img2embed ? node.getAttribute("_url") : node.getAttribute("src"), node.width, node.height,node.getAttribute("id"), align, img2embed,false);
            node.parentNode.replaceChild(tmpdiv.firstChild, node);
        }
    }
    ...
    me.commands["music"] = {
        execCommand:function (cmd, musicObj) {
            var me = this,objId = 'playerkelRagfMuWSi'+ (new Date()-0),
                str = creatInsertStr(musicObj.url, musicObj.width || 480, musicObj.height || 25,objId, 'none', false, true);
