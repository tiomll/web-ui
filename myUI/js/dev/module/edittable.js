...
$("head").append('<style type="text/css">.edit-cell{background:url(images/girdbtbg.gif) no-repeat bottom left;}\
.edit-cell-save{}\
.edit-cell-success{background:#fff;}\
.edit-cell-error{background:#f00;}\
</style>');
		...
		$.ajax({
			...
			// 成功，执行回调函数
				...
				if( msg ){
					...
					td.text( _value );
					if( op.showError ){dialog.alert({content:msg});}
				}else{
			...
			error: function(xhr, status){
				...
				if( op.showError ){
					if(status === "timeout"){
						dialog.alert({content:"提示：连接超时，保存失败！"});
					}else if(status === "parsererror"){
						dialog.alert({content:"提示：JSON 解析错误，保存失败！"});
					}else{
						dialog.alert({content:"提示：未知错误，保存失败！"});
					}
				}
