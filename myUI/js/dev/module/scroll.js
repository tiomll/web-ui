define(function(require,exports,module){
	var doc = window.document,db = doc.body;
	var mousewheel = 'onmousewheel' in document ? 'mousewheel' : 'DOMMouseScroll';
	var Scroll = function(opts){ return new Scroll.prototype.init(opts);};
	Scroll.prototype = {
		constructor:Scroll,
		//初始化
		init:function(opts){
		...
		recalculated:function(){
			...
			if(this.lScroll){
				if(this.width < W){
					...
					//this.vScroll.style.display = this.lScrollOuter.style.display = 'block';
		...
		addWarpper:function(dir){
			...
			this.wrapper = _createEl('<div class="scroll_warpper" style="position:relative;overflow:hidden;"></div>',this.parent);
		...
		addVscroll:function(){
			...
			this.vScrollOuter = _createEl('<div class="scroll_topOuter" style="position:absolute;top:0;right:0;height:'+this.height+'px;overflow:hidden"></div>',this.wrapper)
			this.vScroll = _createEl('<div class="scroll_top" style="position:absolute;top:0;right:0;"></div>',this.wrapper);
			this.recalculated();
		},
		//对外提供添加横向滚动条的方法
		addLscroll:function(){
			...
			this.lScrollOuter = _createEl('<div class="scroll_leftOuter" style="position:absolute;bottom:0;left:0;width:'+this.width+'px;overflow:hidden"></div>',this.wrapper)
			this.lScroll = _createEl('<div class="scroll_left" style="position:absolute;left:0;bottom:0;"></div>',this.wrapper);
			this.recalculated();
		},
		//删除竖向滚动条
		delVscroll:function(){
